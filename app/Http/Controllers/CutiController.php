...
class CutiController extends Controller
{
    ...
    public function sisaCuti($kode_cuti)
    {
    try {
        $cuti = DB::table('master_cuti')->where('kode_cuti', $kode_cuti)->first();
        
        if ($cuti) {
            // Hitung jumlah cuti yang telah digunakan
            $jumlahCutiDigunakan = DB::table('log_cuti')->where('kode_cuti', $kode_cuti)->sum('jml_hari');
            
            // Hitung sisa cuti
            $sisaCuti = $cuti->jml_hari - $jumlahCutiDigunakan;
            
            return view('cuti.sisacuti', compact('cuti', 'jumlahCutiDigunakan', 'sisaCuti'));
        } else {
            return Redirect::back()->with(['warning' => 'Kode Cuti tidak ditemukan']);
        }
    } catch (\Exception $e) {
